...
import { resolveCanonicalAppOrigin, resolveMagicLinkBaseUrl, resolveRequestOrigin } from "./appOrigin";
...
describe("resolveMagicLinkBaseUrl", () => {
  ...
  it("prefers APP_BASE_URL when configured", () => {
    ...
    expect(resolveCanonicalAppOrigin()).toBe("https://app.example.com");
  });
  ...
  it("uses allowlist first entry when APP_BASE_URL is missing", () => {
    ...
    expect(resolveCanonicalAppOrigin()).toBe("https://first.example.com");
  });
  ...
  it("resolves the current request origin from forwarded headers", () => {
    const origin = resolveRequestOrigin(
      makeRequest({
        host: "localhost:3000",
        "x-forwarded-host": "faa-part-107-prep.vercel.app",
        "x-forwarded-proto": "https",
      })
    );
    expect(origin).toBe("https://faa-part-107-prep.vercel.app");
  });
});
